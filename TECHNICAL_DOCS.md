...
- apps/api/src/logic/signalCorrelation.ts (cross-match signal cluster detection, backend-deduplicated by real fixture id)
...
- **Contrarian** fades signals it classifies as market-only moves (`isMarketOnlyMove`, reusing the exact `fieldPressureScore < 22` threshold `SignalIntelligencePanel.tsx` already labels "MARKET-ONLY MOVE"), taking the opposite side at the real quoted price read from the original `OddsSnapshot` the signal was built from — not a synthesized value, also at a flat 1-unit stake.
- **Kelly Criterion** takes the same side as the signal (a sizing strategy, not a direction strategy) but varies its stake per position using the Kelly formula. Since `confidenceScore` is a quality measure, not a literal win probability, it instead scales an assumed edge over the market's own implied probability (`1/oddsTaken`), capped at `MAX_EDGE = 0.15` (15 percentage points at full confidence). The raw Kelly fraction is capped at `MAX_STAKE_FRACTION = 0.2` (full Kelly can recommend unrealistically large fractions) then scaled by `KELLY_BANKROLL_UNITS = 10` so stakes land in a range comparable to the other two agents' flat bets (0-2 units). At `confidenceScore = 0` the Kelly fraction is exactly 0 for any odds value — a real algebraic property, not an approximation — so a zero-confidence signal always stakes nothing. **Kelly Criterion rejects a position outright, rather than just capping it,** when its raw (uncapped) fraction exceeds `MAX_STAKE_FRACTION` — `exceedsKellyRiskLimit(oddsTaken, confidenceScore)` gates `buildKellyCriterionPosition`, and the rejection reports a `risk_limit_exceeded` reason via the mechanism below.
...
All three settle to `correct`/`incorrect`/`pending` per position and report net units, ROI%, and win rate. `ArenaPosition` carries an explicit `stakeUnits` field (Momentum Follower/Contrarian always `1`; Kelly varies) so ROI stays mathematically correct once one agent's stakes are variable — `roiPercent` divides net profit by the *sum of actual stakes risked*, not `settledCount × 1`. Settlement is tamper-evident: a SHA-256 hash of all three ledgers, and the underlying data can be independently verified via the existing `GET /api/onchain/validate-stat` — zero new on-chain code was needed. Over/Under totals signals are excluded from all three agents (`isTotalsSignal`). A subtle correctness detail found during design: negating a stake for an incorrect position is written as `0 - stakeUnits`, not `-stakeUnits`, so a legitimately-zero Kelly stake settles to `+0`, not `-0` — `Object.is(-0, 0)` is `false`, which Vitest's `toBe()` uses, so this mattered for test correctness once stakes could be exactly zero (the flat-stake agents never hit this, since their stake is always exactly 1). Spec: `docs/superpowers/specs/2026-07-08-arena-kelly-criterion-design.md`.

**Retroactive backtesting** (`apps/api/src/logic/backtest.ts`'s `computeBacktestScoreboards`, `GET /api/arena/backtest`) replays Momentum Follower and Kelly Criterion — reusing their existing builder functions and `arena.ts`'s own `summarize` (exported for this purpose) with zero duplicated logic — against the 500 most recent *settled* entries in the permanent archive, rather than the live, capped-100 `store.signals`. **Contrarian is deliberately excluded**: `buildContrarianPosition` needs the real match's final score to resolve the opposing side's outcome (a signal's own `resultStatus === "incorrect"` is ambiguous between "the opponent won" and "the match was a draw," and only the raw score disambiguates), and neither `ArchiveEntry` nor the archived `AgentSignal` ever captures it — extending the already-shipped, insert-only archive schema to add it was considered and rejected, confirmed with the user, since it would only help newly-archived signals going forward. The response's `note` field surfaces this exclusion explicitly rather than silently omitting the agent. Spec: `docs/superpowers/specs/2026-07-08-arena-archive-backtest-design.md`.

## Meta-Agent Recommendation and Skeptic Check

Rendered directly in `ArenaPanel.tsx`, both read-only, both computed frontend-side from `GET /api/arena`'s existing fields (zero backend changes):

- **Meta-agent recommendation** (`getMetaAgentRecommendation()`) ranks the three Arena agents by ROI% (stake-size normalized, since Kelly Criterion stakes variable amounts while the other two stake flat 1 unit — the prior "Leading" badge ranked by raw `netUnits`, which wasn't a fair comparison). Requires `settledCount >= 5` per agent and at least 2 qualifying agents before declaring a leader, and hedges its language when the gap is under 10 percentage points.
...
`apps/api/src/logic/signalCorrelation.ts` (`GET /api/signal-correlation`) detects when signals fire across 2+ distinct matches close together in time — a cross-match pattern the core signal engine (which only ever reasons about one match) has no visibility into. `findSignalClusters` groups the *entire* stored signal history (`store.signals`, capped 100) via session-windowing: sorted by `createdAt`, a new group starts whenever the gap to the previous signal in the current group exceeds 5 minutes, so a steady trickle of correlated signals can span longer than 5 minutes in total as long as no single gap exceeds it. Only groups spanning 2+ distinct `matchId`s are reported — a single match firing multiple signals in a row is normal, already-covered behavior, not correlation. No severity/type filtering is required to join a cluster; each cluster reports a `severityBreakdown` so significance can be judged directly. Unlike steam detection, the entire history is scanned (not just the trailing run), since multiple distinct clusters can exist at different points across the 100-signal cap.

Computed live at request time from `store.signals`; never mutates `agent.ts`/`store.ts`'s state. Both `matchIds`/`matchCount` on this endpoint and its pattern-matched sibling below are deduplicated by real fixture id (`baseMatchId`, same implementation proven in `signalPerformance.ts`) so multiple Over/Under totals lines firing on the same real match no longer inflate a cluster into a false multi-match positive — a real bug found and fixed 2026-07-11. Rendered on the dashboard via `apps/web/src/components/SignalCorrelationPanel.tsx`. Spec: `docs/superpowers/specs/2026-07-08-signal-correlation-design.md`.

**Pattern-matched correlation** (`GET /api/signal-correlation/patterns`, `findPatternMatchedClusters`) is a stricter, separate detection pass: it only reports a cluster when the *same* pattern — `side` ("direction"), `severity`, and market (`1x2`/`totals`, via the existing `isTotalsSignal` classifier already used by `arena.ts`) — repeats across 2+ distinct matches within the window, rather than any signals firing close together regardless of what they say. `signalType` is deliberately excluded from the pattern key, since it's already a deterministic function of `severity` in `signalEngine.ts` (`HIGH→SHARP_MOVE`, `MEDIUM→MOMENTUM_SHIFT`, `LOW→WATCH`) — including both would double-count the same axis. Implemented by partitioning signals by pattern key first, then reusing the exact same session-windowing algorithm independently within each partition (extracted into a shared `sessionWindowGroups` helper, reused by the original `findSignalClusters` too — regression-tested to confirm the extraction changed zero existing behavior). Spec: `docs/superpowers/specs/2026-07-09-pattern-matched-signal-correlation-design.md`.

## Composite Confidence Score and Signal-Type Performance

Every signal now carries a `confidenceScore` (0-100, optional field on `AgentSignal`), computed by a new `calculateConfidenceScore` function in `signalEngine.ts` alongside the existing `calculateMomentumScore` — separate from and never changing `severity`/`momentumScore`'s existing, tested behavior. Blends three weighted components: **magnitude** (weight 0.5, normalized against the existing 15% HIGH severity threshold), **field pressure** (weight 0.3, normalized against `marketMaker.ts`'s `FIELD_PRESSURE_MAX`, now exported rather than duplicated), and **freshness tightness** (weight 0.2, a new graduated companion to the existing pass/fail freshness gate — `computeFreshnessTightness` in `scoresContextFreshness.ts` reports how close in time the attached `scoresContext` actually is on a 0-100 scale, rather than just whether it passed the existing 60-second gate). Weights renormalize among only the available components when `scoresContext` is absent, so a signal with no field context is scored on magnitude alone, never penalized for missing data it never had a chance to have.

Historical hit-rate per signal type is a separate, async, archive-backed concern: `GET /api/signal-performance` (`logic/signalPerformance.ts`'s `summarizeSignalTypePerformance`) reads the 500 most recent settled entries from the archive and reports accuracy per `signalType`. Deliberately kept out of the synchronous signal-creation loop — querying Supabase per signal would introduce real latency/complexity into the one piece of core pipeline code that's stayed fully synchronous all session, and can't be verified end-to-end in this dev environment (no live Supabase credentials here). Fail-open is inherited automatically from the existing `getArchivedSignals`. Spec: `docs/superpowers/specs/2026-07-08-composite-confidence-score-design.md`.

Rendered on the dashboard via `apps/web/src/components/SignalPerformancePanel.tsx` — one card per signal type (sorted by settled count, most statistically meaningful first), color-coded by accuracy threshold. Confirmed with real production data at build time: WATCH 88% (52 settled), MOMENTUM_SHIFT 87% (23 settled), SHARP_MOVE 33% (27 settled) — an honest, not-cherry-picked track record left fully visible rather than only showing favorable numbers. Spec: `docs/superpowers/specs/2026-07-09-signal-performance-panel-design.md`.

**Match-diversity metrics (`distinctMatchCount`, `largestMatchSharePct`).** Investigating that same real data found the three accuracy figures above were each dominated by a single match (89-100% concentration — see `PROJECT_STATE.md`'s "Open questions") with nothing in the API surfacing it. Both fields are computed by the same `summarizeSignalTypePerformance` from the same already-settled, already-grouped data — `distinctMatchCount` is how many distinct real matches a signal type's settled entries span, `largestMatchSharePct` is what fraction come from its single most-represented match. Totals sub-market matchIds (`<fixtureId>-totals-<line>`) collapse to their base fixture before counting, so six correlated lines on one real match don't inflate the diversity count. Spec: `docs/superpowers/specs/2026-07-09-signal-performance-match-diversity-design.md`.

**Confidence-bucketed performance (`GET /api/signal-performance/by-confidence`, `summarizeConfidenceScorePerformance`).** `confidenceScore` was designed to be more informative than raw `severity`/`signalType` — it blends field pressure and freshness into the score — but nothing measured whether it actually predicts accuracy better. Buckets settled signals by `confidenceScore` range (`0-25`/`25-50`/`50-75`/`75-100`) instead of `signalType`. Checked before building: **every currently-settled archived signal predates `confidenceScore`'s introduction (item #7)** — confirmed directly on a sample entry's timestamp — so this endpoint returns an empty array today by design, not a bug. Built anyway (confirmed with the user): cheap, well-tested, and fills in naturally as the remaining tournament matches settle. Entries missing `confidenceScore` are excluded entirely; empty buckets are omitted rather than shown with a 0%/NaN placeholder; buckets are returned in ascending order (unlike the signalType sibling, which has no natural order). Rendered on the dashboard via `apps/web/src/components/ConfidenceCalibrationPanel.tsx`. Spec: `docs/superpowers/specs/2026-07-09-confidence-bucketed-performance-design.md`.

**Longshot confidence penalty.** Investigating 294 real settled archive rows found accuracy correlates almost perfectly with the underlying decimal odds level, not the percentage-compression magnitude severity is based on — 1X2 accuracy broke from 60% to 0% right at `oddsAfter=3` (most incorrect longshot signals fired on an already-losing team deep in a match). `LONGSHOT_ODDS_THRESHOLD = 3` / `LONGSHOT_CONFIDENCE_FACTOR = 0.3` (both real, data-derived values, not invented — documented directly in a code comment above the constants, including the sample-size caveat) apply a multiplicative penalty to `calculateConfidenceScore` only when `oddsAfter >= 3`; every non-longshot signal's score is unchanged. Kelly Criterion's stake sizing shrinks automatically on longshot signals as a free second-order effect, with zero changes to `arena.ts`.
...
**Stale-finished-match repolling (fixed 2026-07-08).** `prioritizeLikelyLiveFixtures()` only re-ranks fixtures by a `StartTime` heuristic — it never filtered any out. If fewer than 14 fixtures looked "likely live," a long-finished fixture could still fill a rotation slot and get fully reprocessed every cycle, eventually producing a "new" `AgentSignal` for the exact same historical tick once its `OddsSnapshot` aged out of the shared 800-entry cache and `signalAlreadyExists`'s 6-hour dedup window had passed. Fixed with a new `filterOutConfirmedFinishedFixtures(fixtures, priorMatchesById)` in `txlineClient.ts`, called in `fetchTxLineFeed()` before `prioritizeLikelyLiveFixtures()` runs: it excludes any fixture whose ID matched a `Match` with `status === "finished"` in the *previous* cycle's `store.matches` (which `agent.ts` doesn't overwrite until after `fetchTxLineFeed()` returns, so the prior cycle's confirmed statuses are still there to read — no new data-fetching needed). Spec: `docs/superpowers/specs/2026-07-08-stale-finished-match-repolling-fix-design.md`, plan: `docs/superpowers/plans/2026-07-08-stale-finished-match-repolling-fix.md`.

**Replay-path draw and totals settlement bugs (found and fixed 2026-07-11).** `server.ts`'s `/api/replay/backtest` route had a separate, duplicate settlement implementation (extracted into `logic/replaySettlement.ts`) that was missing a draw-outcome branch (mirroring the same bug already fixed in `store.ts`'s live-path settlement) and compared a totals signal's raw `matchId` against a set of base fixture ids, so a totals signal's replay-path settlement could never resolve even after its real fixture finished. Both fixed in the new shared module; `server.ts`'s sibling `/api/recent-results` route had the identical matchId-bucketing bug and was fixed the same way.

**ArenaPanel Kelly Criterion wiring gap (found and fixed 2026-07-10).** `GET /api/arena` has returned three agent scoreboards (`momentumFollower`, `contrarian`, `kellyCriterion`) since the Kelly Criterion strategy shipped, but `ArenaPanel.tsx`'s response type only declared the first two and its render logic never referenced the third — Kelly Criterion's results were computed correctly server-side but silently dropped on the dashboard. Fixed by adding `kellyCriterion`/`stakeUnits`/the `kelly_criterion` agentId variant to the frontend's local types, adding a third `ScoreboardCard`, switching the grid to three columns, generalizing the leader-detection logic from a two-way to an n-way comparison, and updating the panel's heading/description to mention all three agents.

## Deployment Incidents Found and Fixed

Two production deployment incidents were investigated and resolved this session — neither was a code bug, but both blocked judges from seeing merged work on the live site.

**Vercel deploy pipeline disconnected from Git (2026-07-09).** The Signal Archive and Signal Performance dashboard panels were confirmed present in `App.tsx` on `main`, yet neither appeared on the live production frontend. Investigated by fetching the deployed JS bundle and comparing it byte-for-byte against from-scratch builds of specific historical commits: the live bundle was an exact match for the commit immediately *before* the Signal Archive panel was even merged — production had been frozen there through every subsequent push. Two stacked root causes: (1) the Vercel project had no Git connection at all — every prior deployment (~20+) was a manual `vercel deploy` CLI snapshot with zero commit metadata, so pushes to `main` had nothing wired up to receive them; fixed by connecting the Vercel project to GitHub via Project Settings → Git. (2) Once connected, the first real git-triggered build failed (`sh: line 1: vite: command not found`, exit 127) because Project Settings → Root Directory was unset and defaulted to the repo root instead of `apps/web`, where `vite` and the frontend's `node_modules` actually live; fixed by setting Root Directory to `apps/web`. Verified live afterward: both panels render with real production data, and auto-deploy on push to `main` is now confirmed working end-to-end.

**Render bandwidth suspension (2026-07-10).** The backend went down; `/health` stopped responding. Render's usage dashboard showed the Hobby workspace's free 5GB/month bandwidth allowance exhausted (6GB/5GB), with "Service-Initiated" traffic (5.59GB) — not HTTP responses to end users (421MB) — the overwhelming majority of the overage. Render's documented behavior: exceeding free bandwidth with no payment method on file suspends all free services for the rest of the calendar month, which would have kept the backend down until August 1, past the July 19 deadline, if left as-is. Fixed by adding a card to the Render workspace; overage is billed at $0.15/GB (trivially cheap at the current overage level). Service confirmed back to "Deployed" status and `/health` responding `{"ok":true,"status":"running"}`. Flag for the rest of the tournament: "Service-Initiated" bandwidth is the dominant cost driver here, not user traffic — if usage patterns change (more frequent polling, more push-stream volume), watch the Render billing page to avoid a repeat before the 19th.
...
Every signal reports `oddsChangePct` (raw percentage odds compression) and, since TxLINE's feed is already de-vigged at the source (see "Reliability Filter"/Automated Test Coverage sections), an additional optional `probabilityPointShiftPct` — a de-vigged implied-probability-point shift (`(1/oddsAfter - 1/oddsBefore) * 100`), same sign convention as `oddsChangePct` but a genuinely different number. Surfaced in the signal's `explanation` text as a distinct sentence; no dedicated UI panel, by design.
...
Following an external technical review, a set of production-readiness items were investigated and, where a real gap existed, fixed:

- **CI**: `.github/workflows/ci.yml` runs backend tests+build and frontend lint+build in parallel on every push/PR to `main`.
- **Pinned dependencies**: all `"latest"` version specifiers in `apps/api/package.json` (15 entries) pinned to caret-range versions matching what was already installed; `apps/web/package.json` was already fully pinned.
...
- **LICENSE**: MIT license added at the repo root.
- **Idempotency**: `signal_archive`/`match_archive` use `.upsert(..., { ignoreDuplicates: true })` instead of a plain insert, guarding against a real restart-timing race (a crash during the periodic-snapshot save window followed by restart from a slightly-stale snapshot could re-detect and re-archive an already-archived signal/match). Confirmed in production: a genuine batch of pre-fix duplicate rows was found and deduplicated before the constraint was added.
- **Bounded queues**: every in-memory array was audited and confirmed already capped (`oddsSnapshots` 800, `signals` 100, `agentRuns` 50, `recentFinishedMatches` 20) — no gap found, no code change needed.
...
A hand-written OpenAPI 3.0.3 spec at the repo root (`openapi.yaml`) documents all 24 backend endpoints with full schema detail, including the deeply nested `/api/replay/backtest` response, the `X-API-Key` security scheme, and both rate limits. Served live via `swagger-ui-express` + `yamljs` at `GET /api/docs` — a public GET route like any other, so a judge can browse and execute real requests against the live API with zero setup. Validated with `npx @redocly/cli lint openapi.yaml` (0 errors; only cosmetic warnings remain, e.g. missing `operationId` fields).
